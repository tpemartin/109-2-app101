...
```{r imgX0}
# 取得第一個古蹟的圖片網址
get_ImgInfo <- function(x){
   targetUrl <- heritageData$representImage[[x]]
targetUrlencoded <- URLencode(targetUrl)
# 讀入圖片
library(magick)
magick::image_read(targetUrlencoded) -> imgX0
magick::image_info(imgX0)
}

purrr::map(1:20,
           get_ImgInfo) -> list_info 
purrr::transpose(list_info) -> tr_list_info
```
...
```{r version2}
   targetUrl <- heritageData$representImage[[3]]
targetUrlencoded <- URLencode(targetUrl)
# 讀入圖片
library(magick)
magick::image_read(targetUrlencoded) -> imgX0
...
```{r}
fourCorners <- list(
   paste0(width, "X", height, "+", 1*width,"+", 0*height),
paste0(width, "X", height, "+", 1*width,"+", 1*height),
paste0(width, "X", height, "+", 0*width,"+", 0*height),
paste0(width, "X", height, "+", 0*width,"+", 1*height)
)
fourCorners
```
...
```{r version2}
##分成四部分
right_up <-image_crop(imgX,fourCorners[[1]]) #右上

right_down <- image_crop(imgX,fourCorners[[2]]) #右下

left_up <- image_crop(imgX,fourCorners[[3]]) #左上

left_down <- image_crop(imgX,fourCorners[[4]]) #左下
