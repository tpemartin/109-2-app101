...
table(is.na(outofcontrol_latitude))
```
...
heritageData[(h_lat > 27)& (!is.na(h_lat)), c("longitude","latitude")] <- heritageData[(h_lat > 27)& (!is.na(h_lat)), c("latitude", "longitude")]
...
exampleList[c("b","a")] <- exampleList[c("a","b")]
exampleList
```
```{r}
char1 <- c("david", NA, "john")
num <- c(23, 454, NA)
char2 <- c("david", NULL, "john")
char1
char2
```
...
right_lati <- wrong_long <- heritageData$longitude[limit_1 & na_data]

right_long <- wrong_lati <- heritageData$latitude[limit_1 & na_data]
```
...
```{r version3}
heritageData$longitude[limit_1 & na_data] <-right_long

heritageData$latitude[limit_1 & na_data] <-right_lati

# 修正後的錯誤資料

heritageData$longitude[limit_1 & na_data]

heritageData$latitude[limit_1 & na_data]
